...
## Definição
   Na falta de uma grande base de dados de teste, utilizada para 
  ...
  se comporta ao receber observações não familiares.. 
...
```{r, fig.cap="Exemplos da variação da quantidade de graus de liberdade do suavizador.", fig.width=6, fig.height=5}
library(tidyverse)
library(splines)
...
Neste caso, podemos ver que para os dados de treino, aumentar a quantidade
...
os graus de liberdade pode ser encontrado com:
...
Ou seja, 14 graus de liberdade levaram à menor taxa de erro durante o 
processo de validação cruzada. Com isso, o método nos ajudou a encontrar
um bom valor para os graus de liberdade do modelo. 